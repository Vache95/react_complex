  ...
  return (
    <div className='product-card'>
      ...
      <div className='product-card__bottom'>
        ...
        {!productBuy?.includes(product?.id) ? (
          <Button className='button-buy' onClick={() => setProductBuy([...productBuy, product?.id])}>купить</Button>
        ) : (
          <div className='product-card__bottom-buy'>
            <Button
              className='button-minus'
              onClick={() => dispatch(reduceProduct(selectProducts[product?.id]?.selectProductCost ? { productId: product?.id, productPrice: product?.price } : {}))}
            >-</Button>
